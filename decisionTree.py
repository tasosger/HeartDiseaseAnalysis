...
def partition_entropy_by(inputs: List[Any], attribute: str, label_attribute:str)->float:
    ...
    labels = [[getattr(input,label_attribute) for input in partition] for partition in partitions]
...
def build_tree_id3(inputs: List[Any], split_attributes: List[str], target_attribute: str)-> DecisionTree:
    label_counts = Counter(getattr(input, target_attributes) for input in inputs)
    ...
    subtrees = {attribute_value: build_tree_id3(subset, new_attributes, target_attribute) for attribute_value, subset in partition.items() }
